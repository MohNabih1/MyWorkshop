...
tuple<string,string> make_meta ( string v, gtype tp ) {
  if (*tp.name == "int")
    return tuple<string,string>("new (GC) NInt("+v+")","");
  else if (*tp.name == "string")
    return tuple<string,string>("new (GC) NString("+v+")","");
  else if (*tp.name == "float")
    return tuple<string,string>("new (GC) NFloat("+v+")","");
  else if (*tp.name == "bool")
    return tuple<string,string>("new (GC) NBool("+v+")","");
  else if (tp.params == nullptr || tp.params->size() == 0)
    return tuple<string,string>("("+v+"==nullptr)?nullptr:("+v+"->meta())","");
  else if (tp.params != nullptr && tp.params->size() > 0) {
    if (*tp.name == "list")
      return tuple<string,string>("new (GC) NId(\"[...]\")","");  // to avoid recursive env
    else if (*tp.name == "vector" || *tp.name == "list") {
      string nv { new_var() };
      string vs { new_var() };
      auto t { make_meta(nv,*(*tp.params)[0]) };
      string gs { "   vector<Node*>* "+vs+" = new (GC) vector<Node*>();\n" };
      gs += "   for ( auto "+nv+": "+v+" ) {\n";
      gs += get<1>(t)+"      "+vs+"->push_back("+get<0>(t)+");\n   }\n";
      return tuple<string,string>("new (GC) NVec("+vs+")",gs);
      ...
      e += " }";
      return tuple<string,string>("new (GC) NTuple(new (GC) vector<Node*>("+e+"))",gs);
    } else return tuple<string,string>("new (GC) NId(\""+*tp.name+"\")","");
...
prog: { hout << "#include \"Node.h\"\n\n";
	...
	for ( char c: string(include_file) )
	  ...
	  hout << "typedef " << get<0>(x) << "_* " << get<0>(x) << ";\n";
          for ( auto c: *get<2>(x) ) {
            hout << "class " << c << "_;\n";
	    hout << "typedef " << c << "_* " << c << ";\n";
	  }
        ...
        for ( auto x: abstract_classes ) {
          ...
          if (get<2>(x)->empty()) {
            hout << "Node* meta () const;\n";
            hout << "bool operator== ( const " << get<0>(x) << "_ &that ) const;\n";
          } else {
            hout << "virtual Node* meta () const = 0;\n";
        ...
        for ( auto x: abstract_classes ) {
          if (get<2>(x)->empty()) {
            ...
            sout << " ) {\n   " << get<0>(x) << " o = new (GC) " << get<0>(x) << "_();\n";
            hout << " );\n";
            for ( auto attr: *get<1>(x) )
              sout << sf("   o->%s = %s;\n",get<0>(*attr)->c_str(),get<0>(*attr)->c_str());
            sout << "   return o;\n};\n";
            sout << "Node* " << get<0>(x) << "_::meta () const {\n";
            if (get<1>(x)->size() > 0) {
              string gs = "";
              string e = "{ ";
              int i = 0;
              ...
              e += " }";
              sout << gs << "   return new (GC) NCall(\"" << get<0>(x)
                   << "\",new (GC) vector<Node*>("+e+"));\n};\n\n";
            } else sout << "return new (GC) NId(\"" << get<0>(x) << "\");\n};\n";
...
cdef: class_token ID ':' ID '(' attrs ')'
	  ...
	  auto tpn { get<1>(*attr)->toString(true) };
        ...
        for ( auto attr: *$6 )
          sout << sf("   o->%s = %s;\n",get<0>(*attr)->c_str(),get<0>(*attr)->c_str());
        sout << "   return o;\n};\n\n";
        out << sf("inline %s %s""_::to%s () const { return dynamic_cast<%s""_*>((%s)this); }\n\n",
